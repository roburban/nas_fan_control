...
# This script is based on the PID_fan_control.ps of Kevin Horton, but with some
# significant changes:
#   - support for motherboards that do not have fan "zones", such as the ASRock Rack X470D4U2-2T
#       - for such boards the individual HD fans can have a "factor" associated with them
#         which allows the duty-cycle to be modified for some fans (see @asrock_zones)
#   - much code cleanup: no warnings/errors reported with "use strict; use warnings;"
#   - all variables declared as local or global
#   - removed calls to non-essential external programs ([e]grep, awk, sed), and replaced with
#     pure Perl
#   - reformatting of code and comments to fit on narrower terminals
#   - added capability to become a daemon (depends on Perl package Proc::Daemon)
#   - added a wrapper function to call external programs (depends on IPC::Run)
# 
# The script keeps the idea of addressing subsets of fans using the concept of zones, which
# means there are basically no changes in "supermicro" mode. In "asrock" mode, the zones
# are defined explicitly in @asrock_zones.
...
my $config_file = '/root/etc/PID_fan_control_config.ini';
...
my @asrock_zones = (
    { FAN1 => { index => 0 } },  # CPU
    {
        ...
        FAN3 => { index => 2 },
        FAN4 => { index => 3 },
        FAN5 => { index => 4, factor => 1.1 },  # run this one 10% faster
        FAN6 => { index => 5, factor => 1.1 },  # run this one 10% faster
    }, # HD
);
...
my $hd_num_peak = 2;        # Number of warmest HDs to use when calculating average temp
my $hd_fan_duty_start = 32; # HD fan duty cycle when script starts
...
#---------------------------------------------------------------------------
my $high_cpu_temp = 75;    # will go HIGH when we hit
my $med_cpu_temp  = 68;    # will go MEDIUM when we hit, or drop below again
my $low_cpu_temp  = 40;    # will go LOW when we fall below 35 again
...
my $hd_fan_duty_low      = 16;     # some 120mm fans stall below 30.
...
my $hd_fan_header  = "FAN2";   # used for printing to standard output for debugging
my @hd_fan_list = qw(FAN2 FAN3 FAN4 FAN5 FAN6);    # used for logging to file
...
if ($Daemon) {
    my $pid = Proc::Daemon::Init(
        work_dir => '/tmp',
        pid_file => '/var/run/PID_fan_control.pid',
    );
    exit(0) if ($pid > 0);
}
...
sub get_hd_list {
    ...
    foreach (run_command(@cmd)) {
        next if (/SSD|Verbatim|Kingston|Elements|Virtual/);
        if (/\((?:pass\d+,(a?da\d+)|(a?da\d+),pass\d+)\)/) {
            my $disk = defined($1) ? $1 : $2;
            dprint(2, $disk);
            push(@vals, $disk);
        }
    }
